...
# Tool Selection
tool_mode = st.sidebar.radio("Tool Mode", ["Draw ROI", "Edit/Move ROIs"])
...
with col1:
    if st.button("Copy Previous ROI"):
        ...
        if curr_idx > 0:
            ...
            if prev_rois:
                ...
                # Force regeneration of drawing state for this image
                if selected_name in st.session_state.drawing_states:
                    del st.session_state.drawing_states[selected_name]
                
                # Signal to regenerate canvas input
                st.session_state.last_selected_name = None 
                
                st.toast(f"Copied {len(prev_rois)} ROIs from previous image.")
                st.rerun()
            else:
                st.warning("Previous image has no ROIs.")
        else:
...
st.write(f"Annotated **{annotated_count}/{n_images}** images.")
        ...
        for r_idx, (x1, y1, x2, y2) in enumerate(file_rois):
            ...
            if region.size > 0:
                ...
                if color_space == "RGB":
                    ...
                    frame_data[f"Val1_Area_{r_idx+1}"] = val1 # Red
                    frame_data[f"Val2_Area_{r_idx+1}"] = val2 # Green
                    frame_data[f"Val3_Area_{r_idx+1}"] = val3 # Blue
                else: # HSV
                    h_raw = mean[0]
                    s_raw = mean[1]
                    v_raw = mean[2]
                    ...
                    frame_data[f"Val1_Area_{r_idx+1}"] = val1 # Hue
                    frame_data[f"Val2_Area_{r_idx+1}"] = val2 # Sat
                    frame_data[f"Val3_Area_{r_idx+1}"] = val3 # Val
...
if "results" in st.session_state:
    ...
    area_cols = [c for c in res_df.columns if c.startswith("Val1_Area_")]
    n_areas = len(area_cols)
    
    for a in range(n_areas):
        suffix = f"Area_{a+1}"
        if f"Val1_{suffix}" in res_df:
            ax.plot(t, res_df[f"Val1_{suffix}"], color=colors[0], linestyle=styles[0], marker='o', label=f"{labels[0]} Area {a+1}", alpha=0.7)
        if f"Val2_{suffix}" in res_df:
            ax.plot(t, res_df[f"Val2_{suffix}"], color=colors[1], linestyle=styles[1], marker='s', label=f"{labels[1]} Area {a+1}", alpha=0.7)
        if f"Val3_{suffix}" in res_df:
            ax.plot(t, res_df[f"Val3_{suffix}"], color=colors[2], linestyle=styles[2], marker='^', label=f"{labels[2]} Area {a+1}", alpha=0.7)
